...
export async function getDashboard(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const data = await storeOwnerService.getDashboard(req.user.userId);
    if (!data) {
...
export async function getRatings(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const ratings = await storeOwnerService.getRatingsForStore(req.user.userId);
...
export async function getProfile(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    const profile = await storeOwnerService.getProfile(req.user.userId);
    if (!profile) {
